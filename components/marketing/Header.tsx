  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "border-b border-border bg-bg-card/80 backdrop-blur-xl"
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-3 py-2 text-[13.5px] text-text-secondary transition-colors duration-150 hover:bg-bg-card hover:text-text-primary"
            >
              {item.label}
            </Link>
          ))}
